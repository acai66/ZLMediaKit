...
RUN apt-get update && \
         ...
         apt-get clean -y && \
         wget https://github.com/cisco/libsrtp/archive/v2.5.0.tar.gz -O libsrtp-2.5.0.tar.gz && tar xfv libsrtp-2.5.0.tar.gz && \
         cd libsrtp-2.5.0 && ./configure --enable-openssl && make -j $(nproc) && make install && \
